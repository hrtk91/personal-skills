...
description: personal-skills repositoryのruntime cloneをWindowsとWSLの両方で安全に更新する。runtime cloneが古い、自動更新のsystemd user timerやWindows Task Schedulerを設定・確認したいときに使う。開発checkoutとは別のclean runtime cloneを更新し、fast-forwardできない、base branch以外、未コミット変更がある場合は更新せず診断する。skillやruleの導入はharnessctl applyで行う。
---

# Personal Skills Auto Update

開発checkoutとは別のclean runtime cloneをfast-forwardし、最新状態を保つ。runtime cloneの更新だけを行い、skillやruleの導入は自動実行しない。利用するprofileの反映は、ユーザーが明示的に`harnessctl apply <profile>`を実行して行う。base branchへのローカル固有コミット、強制更新、stash、resetを行わない。
...
どちらもOSごとのユーザーデータ領域へruntime cloneを作り、15分間隔でone-shot更新を呼ぶ。二重実行はlockで防ぐ。定期実行はruntime cloneの更新だけを行い、profileを適用しない。設定後はtimerまたはtaskの実在、次回実行、直近結果を確認する。
...
同じruntime repoを指す既存タスクが旧形式のActionを持つ場合は、更新成功後にheadless形式へ更新する。別のrepoを指す同名タスクは変更しない。
