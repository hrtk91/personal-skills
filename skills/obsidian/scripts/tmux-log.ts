...
function getLogsByDate(dateStr: string): string[] {
...
function readLogs(logFiles: string[]): string {
...
function isCommandPrompt(line: string): boolean {
  // 一般的なプロンプトパターン
...
function cleanLogContent(content: string): string {
  ...
  return content
    ...
    .filter((line) => {
      ...
      // トークン情報行は除去
      if (/tokens\s*$/.test(line)) return false;
      // ステータスバー的なものは除去
      if (/🤖.*Opus.*│/.test(line)) return false;
      if (/files\s+\+\d+\s+-\d+/.test(line)) return false;
      // Waiting/Thinking系は除去
      if (/^[✳✢·⏺]?\s*(Waiting|Thinking|Running|Spelunking|Percolating|Pouncing|Osmosing)/i.test(line.trim())) return false;
      return true;
    })
    .join("\n");
}
...
function parseLogsToWorkUnits(content: string): WorkUnit[] {
