...
description: 問題解決、設計判断、障害分析、調査方針の立案で、解決策を急ぐ前に「何を問題として調べるか」を整え、独立した複数の探索担当に原因候補を出させ、重複を整理し、反対の証拠と最小の確認方法から次の一手を決めたいときに使う。「そもそも何が問題か」「仮説を広く深く出したい」「調査の筋を立てたい」「次に何を確認すべきか」を扱うときに使う。
---

# 仮説を整理して次の一手を決める

解決案を先に選ばない。低い推論量の探索担当（leaf subagent）が、互いの答えを見ずに原因候補を出す。まとめ役の親agentが、それらを整理し、反対の証拠を探し、次の一手を決める。

始める前に、まとめ役が十分な推論量で動いているか確認する。確認できない場合は、統合と反証だけを高い推論量の `architect` に相談するか、「結論の深さは保証できない」と出力に書く。`architect` は候補を追加で起動しない。候補の起動と最終判断は、常にまとめ役が行う。
...
まとめ役が見る観点を決める前に、fresh context の探索担当を2つ起動する。両者には、確定事実、**問いに偏らない決定の種類**（例: 最も安い確認を選ぶ、go/no-goを決める）、制約、一次資料だけを渡す。いまの問い、別の問い、まとめ役が考えた見る観点は渡さない。決定の種類に原因候補や、いまの問いの言い換えを書かない。
...
問い直し担当と見落とし担当を含め、第一波は4〜6人の探索担当にする。通常は読み取り専用の `explorer` を使い、起動時に実際の推論量とsandboxを確認する。探索担当はfresh contextで起動し、まとめ役だけが起動する。fresh context、低い推論量、読み取り専用のどれかを確認できない場合は、独立した探索担当を起動しない。

その場合は、まとめ役が順番に考える方式へ切り替えるか、利用者に制約を示して判断を仰ぐ。順番に考える方式では、問い直し担当と見落とし担当の役も含め、出力冒頭に「独立性なし・単一 agent の逐次シミュレーション」と書く。各候補の出所も「親の仮説」と明記する。詳しい確認方法は [subagent-context.md](references/subagent-context.md) に従う。

通常の探索担当には、いまの問い、確定事実、制約、主な見る観点、一次資料への参照だけを渡す。別の問いや、まとめ役の暫定結論、他担当の出力、全履歴は渡さない。主な見る観点は、他を見ることを禁じる境界ではなく、優先順位である。近い観点に重要な兆候があれば、深掘りせず追加で見る候補として報告する。

外部への書き込み、設定変更、破壊的な確認、子subagentの起動は許可しない。渡す情報とpromptの雛形は、[subagent-context.md](references/subagent-context.md)を読む。

通常の探索担当と見落とし担当は、最大2件を次の形で返す。観測事実には `user statement`、`file:line`、`log`、`URL` など確かめられる出所を付ける。出所のない内容は推測として扱う。
